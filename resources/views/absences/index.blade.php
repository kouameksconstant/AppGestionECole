{{-- resources/views/heures/index.blade.php --}}
...
<div class="container mt-5">
    ...
    <h2 class="mb-4 text-center text-primary">
        Liste des Heures pour {{ $professeur->nom }} {{ $professeur->prenom }}
    </h2>
    <p class="text-center">
        Nombre total d'heures assignées : {{ $professeur->matieres->sum(fn($matiere) => $matiere->pivot->nb_heures) }} h
    </p>
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            <table class="table table-bordered table-hover table-striped" id="heuresTable">
                ...
                <tbody>
                    @foreach ($professeur->heures as $heure)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <form action="{{ route('heures.destroy', ['professeur' => $professeur->id, 'heure' => $heure->id]) }}" method="POST" class="d-inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette heure ?')">
                                    @csrf
                                    @method('DELETE')
